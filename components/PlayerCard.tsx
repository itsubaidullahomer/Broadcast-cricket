...
const PlayerCard: React.FC<PlayerCardProps> = ({ player, isActive }) => {
  ...
  return (
    ...
    >
      ...
      {/* Player Image Background if available */}
      {player.imageUrl && (
        <div className="absolute right-0 bottom-0 top-0 w-1/2 overflow-hidden opacity-30 mix-blend-screen pointer-events-none">
             <img src={player.imageUrl} alt={player.name} className="w-full h-full object-cover object-top mask-image-gradient" style={{ maskImage: 'linear-gradient(to right, transparent, black)' }} />
        </div>
      )}
      
      {/* Background decoration (Gradient Overlay) */}
      <div className="absolute right-0 top-0 bottom-0 w-2/3 bg-gradient-to-l from-[#11141a]/90 via-[#11141a]/40 to-transparent z-0"></div>
      ...
      <div className="flex-1 flex items-center justify-between px-6 relative z-10">
          ...
          <div className="flex items-center gap-4 flex-1">
              ...
              <div className="flex flex-col z-20">
                  <div className="flex items-center gap-3">
                    <span className={`text-[36px] font-teko font-bold uppercase leading-[0.85] tracking-wide whitespace-nowrap overflow-hidden text-ellipsis max-w-[280px] ${isActive ? 'text-white drop-shadow-md' : 'text-gray-400'}`}>
                        {player.name}
                    </span>
          ...
          {/* Right: Stats Grid */}
          <div className="flex items-center gap-8 mr-4 z-20"> 
              ...
              <div className="flex flex-col items-center px-6 border-x border-gray-800/50 min-w-[50px]">
